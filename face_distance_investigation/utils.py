...
def timesince(time, percent_done):
    ...
    try:
        ...
        print(
            ...
            + (time + ((now - time) / percent_done) * 100).strftime("%H:%M:%S")
        )
    except:
...
def print_updates_get_encodings(
    person, number_of_people_to_scan, encodings_start_time, lists_of_images
):
    """
    ...
    print(
        "Person number "
        + str(len(lists_of_images["people_attempted"]) + 1)
        + " of "
        + str(number_of_people_to_scan)
        + ", "
        + str(
            round(
                100
                * (len(lists_of_images["people_attempted"]))
    ...
    timesince(
        ...
        100 * (len(lists_of_images["people_attempted"])) / number_of_people_to_scan,
    )
...
def find_which_people_and_images_to_scan(attempting_all, lfw_path, image_no_max):
    ...
    people_directory_full_paths = [
        os.path.join(lfw_path, person)
    ...
    people_image_nos = [
        len(
            [
                ...
                if (
                    (".db" not in file)
                    and (os.path.join(person_directory, file) not in IMAGES_TO_EXCLUDE)
                )
            ]
        )
        for person_directory in people_directory_full_paths
    ]
...
def get_this_persons_encodings(person_path, IMAGES_TO_EXCLUDE):
    ...
    images_this_person_in_IMAGES_TO_EXCLUDE = [
        image for image in images_of_this_person if (image in IMAGES_TO_EXCLUDE)
    ]
    images_this_person_not_in_IMAGES_TO_EXCLUDE = [
        image for image in images_of_this_person if (image not in IMAGES_TO_EXCLUDE)
    ]

    face_rec_loaded_images_of_this_person = [
        [image, face_recognition.load_image_file(image)]
        for image in images_this_person_not_in_IMAGES_TO_EXCLUDE
    ]
    encodings_images_this_person_not_in_IMAGES_TO_EXCLUDE = [
        [image[0], face_recognition.face_encodings(image[1])]
        for image in face_rec_loaded_images_of_this_person
    ]
...
def check_for_no_or_multiple_images_in_photo(
    encodings_all_images_this_person, images_of_this_person
):
    """
    ...
    number_faces_found_in_each_image = [
        len(encodings_list[1]) for encodings_list in encodings_all_images_this_person
    ]
    images_without_faces = [
        (number == 0) for number in number_faces_found_in_each_image
    ]

    image_paths_without_faces = [
        images_of_this_person[i]
        for i in range(len(images_of_this_person))
        if images_without_faces[i]
    ]

    encodings_for_images_with_faces = [
        encodings_all_images_this_person[i][1]
        for i in range(len(images_of_this_person))
        if (not images_without_faces[i])
    ]
    paths_for_images_with_faces = [
        images_of_this_person[i]
        for i in range(len(images_of_this_person))
        if (not images_without_faces[i])
    ]
...
def select_right_face_encodings_from_each_image(
    ...
    for i in range(len(paths_for_images_with_faces)):
        ...
        if len(this_image_encodings) == 1:
            ...
            selected_encodings_from_this_persons_images.append(
                selected_encodings_from_this_image
            )
        else:
            encodings_other_images = [
                x
                for x in encodings_for_images_with_faces[:i]
                + encodings_for_images_with_faces[i + 1 :]
            ]
            flattened_encodings_other_images = [
                item for sublist in encodings_other_images for item in sublist
            ]
            if (len(flattened_encodings_other_images) > 0) and (
                len(paths_for_images_with_faces) > 2
            ):
                this_image_face_distances_to_other_image_faces = []
                for j, this_encoding_set in enumerate(
                    encodings_for_images_with_faces[i]
                ):
                    distances = face_recognition.face_distance(
                        flattened_encodings_other_images, this_encoding_set
                    )
...
def put_selected_encodings_into_df(
    selected_encodings_from_this_image, paths_for_images_with_faces, person_path
):
    ...
    if n_encodings_this_person > 0:
        ...
        this_persons_encodings["person_path"] = os.path.dirname(
            paths_for_images_with_faces[0]
        )
...
def encodings_builder(
    ...
    print(
        "\n"
        + encodings_start_time.strftime("%Y_%m_%d__%H:%M:%S")
        + " Doing the encodings..."
        + "\n"
    )
    ...
    for person_number, person in enumerate(peoples_faces_to_scan):
        ...
        (
            ...
            images_this_person_not_in_IMAGES_TO_EXCLUDE,
            images_this_person_in_IMAGES_TO_EXCLUDE,
        ) = get_this_persons_encodings(person_path, IMAGES_TO_EXCLUDE)

        lists_of_images["images_attempted"].extend(
            encodings_images_this_person_not_in_IMAGES_TO_EXCLUDE
        )
        lists_of_images["IMAGES_TO_EXCLUDE_excluded"].extend(
            images_this_person_in_IMAGES_TO_EXCLUDE
        )
        ...
        (
            ...
            encodings_for_images_with_faces,
            paths_for_images_with_faces,
        ) = check_for_no_or_multiple_images_in_photo(
            encodings_images_this_person_not_in_IMAGES_TO_EXCLUDE,
            images_this_person_not_in_IMAGES_TO_EXCLUDE,
        )

        lists_of_images["photos_with_no_faces_found_paths"].extend(
            image_paths_without_faces
        )
        ...
        (
            ...
            images_with_unidentifiable_faces,
            paths_for_images_with_faces,
        ) = select_right_face_encodings_from_each_image(
            paths_for_images_with_faces, encodings_for_images_with_faces
        )

        lists_of_images[
            "photos_with_multiple_faces_and_no_other_images_to_compare_with"
        ].extend(images_with_unidentifiable_faces)

        (
            this_persons_encodings,
            this_persons_first_name,
        ) = put_selected_encodings_into_df(
            selected_encodings_from_this_persons_images,
            paths_for_images_with_faces,
            person_path,
        )
        ...
        if this_persons_encodings.shape[0] > 0:
            lists_of_images["images_with_encodings"].extend(
                this_persons_encodings["image_path"].tolist()
            )
        ...
        if os.path.basename(person_path) in ACTUALLY_SAME_PEOPLE.keys():
            lists_of_images["people_names_corrected_to_other_people"].append(
        ...
        lists_of_images["all_peoples_first_names"] = [
            name for name in lists_of_images["all_peoples_first_names"] if (name != "")
        ]
...
def get_number_faces_to_scan(lfw_path, overall_start_time):
    ...
    people_in_lfw_path = [
        os.path.join(lfw_path, person) for person in os.listdir(lfw_path)
    ]
    ...
    file_str_prefix = os.path.join(
        results_directory,
        overall_start_time.strftime("%Y_%m_%d %H_%M_%S_")
        + (
            "_attempting_all_images_"
            if attempting_all
            else ("_attempting_" + str(number_of_people_to_scan) + "_peoples_images_")
        ),
    )
...
def perhaps_print_comparison_counter(
    comparison_counter, total_comparisons, comparisons_start_time
):
    if comparison_counter % 1000000 == 0:
        print(
            "Comparison number "
            + str(comparison_counter)
            + " of "
            + str(round(total_comparisons))
            + " ("
            + str(round(100 * comparison_counter / total_comparisons, 2))
            + "% completed). "
        )
        timesince(
            comparisons_start_time, 100 * (comparison_counter / total_comparisons)
        )
        print("")
...
def encodings_comparer(all_encodings):
    ...
    print(
        comparisons_start_time.strftime("%Y_%m_%d__%H:%M:%S")
    ...
    for image in range(1, len(all_encodings_lol)):
        for image2 in range(0, image):
            ...
            distance = face_recognition.face_distance(
                all_encodings_lol[image][2], all_encodings_lol[image2][2][0]
            )[0]

            comparison_counter += 1
            perhaps_print_comparison_counter(
                comparison_counter, total_comparisons, comparisons_start_time
            )
            ...
            if same:
                same_face_distances.append(
                    (
                        all_encodings_lol[image][1],
                        all_encodings_lol[image2][1],
                        distance,
                    )
                )
            else:
    ...
    print(
        ...
        + " Image comparisons completed!"
    )

    same_face_distances_df = pd.DataFrame(
        same_face_distances, columns=["path1", "path2", "distance"]
    )
    different_face_distances_df = pd.DataFrame(
        different_face_distances, columns=["path1", "path2", "distance"]
    )
...
def plotter(
    ...
    fig.clf()
    xmin, xmax = (
        min(min(same_face_distances), min(different_face_distances)),
        max(max(same_face_distances), max(different_face_distances)),
    )
    ...
    plt.text(
        ...
        str(comparison_counter)
        + " comparisons of \n"
        + str(len(lists_of_images["images_with_encodings"]))
        + " images of \n "
        + str(len(lists_of_images["people_with_encodings"]))
        + " people",
        ha="center",
        va="center",
    ...
    if cumulative != False:
        ...
        title = "Fit results: mu = {}, std = {}, mu = {}, std = {}".format(
            round(mu_sf, 2), round(std_sf, 2), round(mu_df, 2), round(std_df, 2)
        )
...
def roc_auc(
    ...
    y = np.concatenate(
        (
            np.array([1] * len(same_face_distances)),
            np.array([0] * len(different_face_distances)),
        ),
        axis=0,
    )
...
def do_precision_recall(
    ...
    same_faces_binned["same_faces_freq"] = pd.cut(
        same_faces_binned.same_faces, bin_boundaries
    )

    different_faces_binned = pd.DataFrame({"different_faces": different_face_distances})
    different_faces_binned["different_faces_freq"] = pd.cut(
        different_faces_binned.different_faces, bin_boundaries
    )

    precision_recall_table = pd.concat(
        [
            same_faces_binned.same_faces_freq.value_counts(),
            different_faces_binned.different_faces_freq.value_counts(),
        ],
        axis=1,
        sort=True,
    )

    precision_recall_table["same_faces_freq_cum"] = (
        precision_recall_table["same_faces_freq"].cumsum()
        - precision_recall_table["same_faces_freq"]
    )

    precision_recall_table["different_faces_freq_cum"] = (
        precision_recall_table["different_faces_freq"].cumsum()
        - precision_recall_table["different_faces_freq"]
    )
    precision_recall_table["different_faces_freq_anti_cum"] = (
        sum(precision_recall_table["different_faces_freq"])
        - precision_recall_table["different_faces_freq_cum"]
    )
    ...
    precision_recall_table["cutoff"] = [
        ...
        for x in list(precision_recall_table.index)
    ]

    precision_recall_table["precision"] = precision_recall_table[
        "same_faces_freq_cum"
    ] / (
        precision_recall_table["same_faces_freq_cum"]
        + precision_recall_table["different_faces_freq_cum"]
...
def precision_recall(
    ...
    if doing_precision_recall:
        ...
        print(
            datetime.datetime.now().strftime("%Y_%m_%d__%H:%M:%S")
            + " Now doing precision and recall calculations..."
        )
        print("")

        do_precision_recall(
            same_face_distances_df=same_face_distances_df,
            different_face_distances_df=different_face_distances_df,
            file_str_prefix=file_str_prefix,
            bin_boundaries=[0, 0.1]
            + [round(x, 2) for x in np.arange(0.2, 1.1, 0.01)]
            + [1.2, 1.3, 1.4, 1.5, 2],
        ...
        print(
            datetime.datetime.now().strftime("%Y_%m_%d__%H:%M:%S")
            + " Precision and recall calculations completed."
        )

        return precision_recall_start_time


def select_top_unique_combos_and_output_to_csv(
    df, output_str, ascending, file_str_prefix
):
    """
    ...
    df_sorted["people_set"] = df_sorted[["person1", "person2"]].apply(
    ...
    df_sorted["index"] = pd.Series(
        range(1, N_LOOKALIKES_AND_DIFFERENT_LOOKING_SAME_PEOPLE_TO_INCLUDE + 1)
    )
    df_sorted = df_sorted[["index", "path1", "path2", "distance"]]

    df_sorted.to_csv(file_str_prefix + output_str + ".csv", index=False)

    return df_sorted


def output_most_similar_different_people_and_most_different_same_faces(
    ...
    different_face_distances_df_sorted = select_top_unique_combos_and_output_to_csv(
        different_face_distances_df, "_6_lookalikes", True, file_str_prefix
    )

    # Most different photos of the same person
    same_face_distances_df_sorted = select_top_unique_combos_and_output_to_csv(
        same_face_distances_df,
...
def run_outputs(
    ...
    outputs_str += (
        "\nNumber of images excluded because the wrong face gets picked, etc: "
        + str(len(lists_of_images["IMAGES_TO_EXCLUDE_excluded"]))
        + "\n"
    )
    ...
    outputs_str += (
        str(len(lists_of_images["images_attempted"]))
        + " photos of "
        + str(len(lists_of_images["people_with_images_attempted"]))
        + " people remained to attempt.\n"
    )
    ...
    outputs_str += (
        "\n\t"
        + "Faces not found in "
        + str(len(lists_of_images["photos_with_no_faces_found_paths"]))
        + " images."
    )

    outputs_str += (
        "\n\t"
        + "Not sure which face to pick in "
        + str(
            len(
                lists_of_images[
                    "photos_with_multiple_faces_and_no_other_images_to_compare_with"
    ...
    outputs_str += (
        "\n\t"
        + str(len(lists_of_images["images_for_people_names_corrected_to_other_people"]))
        + " photos of "
        + str(len(lists_of_images["people_names_corrected_to_other_people"]))
        + " people combined into other peoples' folders.\n\n"
    )
    ...
    if len(lists_of_images["images_for_people_names_corrected_to_other_people"]) > 0:
        outputs_str += (
            "Images reassigned into other people's folders:\n"
            + "\n".join(
                lists_of_images["images_for_people_names_corrected_to_other_people"]
            )
            + "\n\n"
        )
    ...
    outputs_str += (
        ...
        + time_diff(overall_start_time, encodings_start_time)
        + " on initial prep, counting pictures etc\n"
        + time_diff(encodings_start_time, comparisons_start_time)
        + " on getting encodings\n"
        + time_diff(comparisons_start_time, graph_start_time)
        + " on comparing encodings\n"
        + time_diff(graph_start_time, precision_recall_start_time)
        + " on making graphs\n"
        + time_diff(precision_recall_start_time, completion_time)
        + " on calculation precision_recall_table\n\n"
        + time_diff(overall_start_time, completion_time)
        + " in total"
    )
...
def all_graphs(
    ...
    print(
        "\n"
        + graph_start_time.strftime("%Y_%m_%d__%H:%M:%S")
        + " Doing the graphs..."
        + "\n"
    )
    ...
    if doing_graphs:
        ...
        if CUMULATIVE_GRAPHS:
            ax = plotter(
                fig_names=(
                    "_1_distributions_same_diff_distances_cum",
                    "_2_distributions_same_diff_distances_with_norm_cum",
                ),
                cumulative=1,
                same_face_distances_df=same_face_distances_df,
                different_face_distances_df=different_face_distances_df,
                ...
                file_str_prefix=file_str_prefix,
                lists_of_images=lists_of_images,
            )

        else:
            ax = plotter(
                fig_names=(
                    "_1_distributions_same_diff_distances",
                    "_2_distributions_same_diff_distances_with_norm",
                ),
                cumulative=False,
                same_face_distances_df=same_face_distances_df,
                different_face_distances_df=different_face_distances_df,
...
def combine_face_images(face_images_df, file_str_prefix, image_note_str):
    ...
    for row in face_images_df.itertuples():
        ...
        cv2.putText(
            image1, str(i + 1), (10, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2
        )
    ...
    cv2.imwrite(
        os.path.join(os.path.join(file_str_prefix + image_note_str)), all_images
    )
...
def plot_first_names_wordcloud(file_str_prefix, lists_of_images):
    ...
    print(
        "\n"
        + datetime.datetime.now().strftime("%Y_%m_%d__%H:%M:%S")
        + " Now making names wordcloud..."
    )

    names = [
        name.lower().capitalize() for name in lists_of_images["all_peoples_first_names"]
    ]

    names_string = " ".join(names)
    wordcloud = WordCloud(
        max_words=1000, width=1800, height=600, collocations=False
    ).generate(names_string)
...
def count_images_lfw(lfw_path):
    ...
    files = [
        file
        for file in [os.listdir(os.path.join(lfw_path, folder)) for folder in folders]
        if ".db" not in file
    ]
